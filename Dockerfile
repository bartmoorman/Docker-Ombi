...
RUN apt-get update \
 ...
 && fileUrl=$(curl --silent --location "https://api.github.com/repos/Ombi-app/Ombi/releases" | jq --arg target ${target} --raw-output '.[0].assets[] | select(.name == $target + ".tar.gz") | .browser_download_url') \
